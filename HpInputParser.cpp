 ...
 */
AccessControlList* HpInputParser::getAclByName(int name)
{
    stringstream ss;
    ss << name;
 ...
 * @return pointer to ACL with entered name (ID).
 */
AccessControlList* HpInputParser::getAclByName(const std::string& name)
{
 ...
 */
AccessControlList* HpInputParser::createNewAcl(const std::string& name)
{
 ...
 */
int HpInputParser::parseCommand(const char* str, unsigned& charsExtracted)
{
    switch ( str[0] )
    {
        case 'a':
 ...
 *         CMD_ACL_RULE_REMARK - string begins with command "remark".
 */
int HpInputParser::parseAction(const char* str, unsigned& charsExtracted)
{
 ...
 */
int HpInputParser::parseProtocol(const char* str, unsigned& charsExtracted)
{
    switch ( str[0] )
    {
        case 'a':
        ...
        case 'i':
            if ( strncmp(str, "ip-in-ip", 8) == 0 )
            {
                charsExtracted = 9;
                return PROTO_IP_IN_IP;
            }
            else if ( strncmp(str, "ipv6-in-ip", 10) == 0 )
            {
                charsExtracted = 11;
                return PROTO_IPv4_IPv6;
            }
            else if ( strncmp(str, "ip", 2) == 0 )
            {
                charsExtracted = 3;
                return PROTO_IPv4;
            }
            else if ( strncmp(str, "icmp", 4) == 0 )
            {
                charsExtracted = 5;
                return PROTO_ICMPv4;
            }
            else if ( strncmp(str, "igmp", 4) == 0 )
            {
                charsExtracted = 5;
                return PROTO_IGMP;
            }
 ...
 */
int HpInputParser::parseProtocolByNum(const char* str, unsigned& charsExtracted)
{
    char buffer[8];
    stringstream ss;
    ss.str(str);
 ...
 */
int HpInputParser::parsePort(const char* str, unsigned& charsExtracted, u_int16_t& portStart, u_int16_t& portStop, bool& portNeg)
{
 ...
 */
u_int16_t HpInputParser::parsePortByNum(const char* str, unsigned& charsExtracted)
{
    char buffer[8];
    stringstream ss;
    ss.str(str);
 ...
 */
u_int16_t HpInputParser::parsePortByName(const char* str, unsigned& charsExtracted)
{
    switch ( str[0] )
    {
        case 'b':
        ...
        case 'd':
            ...
            else if ( strncmp(str, "daytime", 7) == 0 )
            {
                charsExtracted = 8;
                return PORT_DAYTIME;
            }
        ...
        case 'r':
            if ( strncmp(str, "radius", 6) == 0 )
            {
                charsExtracted = 7;
                return PORT_RADIUS;
            }
            else if ( strncmp(str, "radius-old", 10) == 0 )
            {
                charsExtracted = 11;
                return PORT_RADIUS_OLD;
            }
            else if ( strncmp(str, "rwho", 4) == 0 )
            {
                charsExtracted = 5;
                return PORT_WHO;
            }
        ...
        case 's':
            ...
            else if ( strncmp(str, "snmp-trap", 9) == 0 )
            {
                charsExtracted = 10;
 ...
 */
auto_ptr< AclRule > HpInputParser::handleAccessList(int number, unsigned position, const char* str)
{
    /* determine ACL type */
 ...
 */
auto_ptr< AclRule > HpInputParser::handleStandardRule(unsigned position, const char* str)
{
 ...
 * @return smart pointer to new rule object.
 */
auto_ptr< AclRule > HpInputParser::handleExtendedRule(unsigned position, const char* str)
{
    auto_ptr< AclRule > tmp_rule(new AclRule(position));
    AclRule& rule = *tmp_rule;
    const char* tmp_buffer = str;
 ...
 */
int HpInputParser::resolveAccessListType(int number)
{
    /* standardny ACL */
 ...
 *         ERROR_FLAG - if the passed number is not standard nor extended ACL number.
 */
int HpInputParser::resolveAccessListType(const char* str, unsigned& charsExtracted)
{
 ...
 */
int HpInputParser::parseAccessListNumber(const char* str, unsigned& charsExtracted)
{
    char buffer[8];
    stringstream ss;
 ...
 */
string HpInputParser::parseAccessListName(const char* str)
{
    int i = 0;
 ...
 * @param charsExtracted reference to variable where the number of extracted characters will be stored.
 */
void HpInputParser::parseAddrAndWildcard(const char* str, IP_ADDRESS& addr, IP_ADDRESS& wildC, unsigned& charsExtracted)
{
    /* address contains "any" */
    if ( strncmp(str, "any", 3) == 0 )
    {
    ...
    else if ( isdigit(str[0]) )
    {
        ...
        stringstream ss;
        ss.str(str);
        ss.getline(tmp, 32, ' ');           /* get IP address */
        charsExtracted = ss.gcount();       /* number of characters extracted by last getline() */
        ...
        if ( strchr(tmp, '/') != NULL )
        {
            ...
            /* IP address */
            ss2.getline(buff2, 16, '/');
            addr = parseIPv4addr(buff2);
            /* network mask */
            ss2.getline(buff2, 16, '/');
            IP_ADDRESS mask = getMask( (unsigned)atoi(buff2) );
            *((u_int32_t*)&wildC) = ~(*((u_int32_t*)&mask));
        }
        /* it is wildcard */
        else
        {
    ...
    {
        throw Exception("There is a hostname in ACL rule definition!");
    }
}
 ...
 */
IP_ADDRESS HpInputParser::parseIPv4addr(const char* str)
{
    IP_ADDRESS address;
 ...
 */
IP_ADDRESS HpInputParser::ipV4WildcardToMask(const IP_ADDRESS& wildcard)
{
    IP_ADDRESS mask;
 ...
 */
void HpInputParser::fillIPv4Range(const IP_ADDRESS& addr, const IP_ADDRESS& wildc, IP_ADDRESS& range_start, IP_ADDRESS& range_stop)
{
...
/**
 * Method constructs network mask from number of mask bits.
 *
 * @param numOfBits number of network mask bits (value 0-32).
 * @return structure IP_ADDRESS containing constructed mask.
 */
IP_ADDRESS HpInputParser::getMask(unsigned numOfBits)
{
    IP_ADDRESS address;

    u_int32_t mask = 0xFFFFFFFF;
    unsigned bits = numOfBits % 33;
 ...
 * @param charsExtracted reference to variable where the number of extracted characters will be stored.
 */
void HpInputParser::skipWhiteChars(const char* str, unsigned& charsExtracted)
{
    unsigned whiteCh = 0;
 ...
 */
void HpInputParser::rollbackStream(std::istream& stream, std::streamsize size)
{
 ...
 */
string HpInputParser::commandToString(int cmd)
{
    switch ( cmd )
    {
